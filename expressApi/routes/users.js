...
router.post('/', checkAuth, async function(req, res, next){
	if(req.user.admin){
		...
		newUser.admin = false;
		newUser.save();
		res.render('message', {h1 : `  Added new user ${newUser.username}`});
    } else {
		res.render('message', {h1 : "Not authorized!"});
		return;
    }
...
router.post('/delete', async function(req,res,next) {
	if(req.user.admin) {
		...
		if(!user){
		res.render('message' , {h1 : "That user doesn't exist!"});
		return;
		}

		res.render('message', {h1: ` User ${user.username} deleted!`});
	} else {
		res.render('message', {h1 : "Not authorized!"});
		return;
	}
...
router.post('/updateUser', async function(req,res,next){
	...
	if(!user){
	  res.render('message', {h1: "No user found with that password"});
	  return;
	}
  
	if(validPassword(req.body.currentPassword, user.salt, user.password)){
	  //user is an admin or only editing their own account
	  if(req.user.admin || user._id == req.user._id){
		user.salt = crypto.randomBytes(32).toString('hex');
		user.password = pbkdf2.pbkdf2Sync(req.body.newPassword, user.salt, 1, 32, 'sha512').toString('hex');
		user.save();
		res.render('message', {h1: "User" + ` ${user.username}'s Password updated!`});
	  }
	} else if(!req.user.admin && user._id != req.user._id){
	  	res.render('message', {h1: "  Unauthorized!"});
	} else 
	  	res.render('message', {h1: "  Invalid Password!"});
});
